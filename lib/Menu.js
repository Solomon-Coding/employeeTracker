    ...
    availableRolesFnc(){
      db.query(this.role.availableRolesQuery, (err, result) => {
        for (let i=0; i<result.length;i++){   
          ...
          this.employee.availableRolesDepartmentId[i]=result[i].d_id;
        }
      }); 
    } 
    ...
    addEmployee(){
      ...
      .then((data) => {
        ...
        this.QueryAdd(this.employee.addEmployeeQuery(prompt.first, prompt.last,
          this.employee.availableRolesId[index1],this.employee.availableRolesDepartmentId[index1], this.employee.employeesId[index2]),
